...
export type ClassMetaDecorator<M, T extends Function> = (
  meta: M
) => ClassDecorator<T>;

export function ClassMetaDecoratorFn<M, T extends Function>(
  manager: MetadataManager<M, T>
): ClassMetaDecorator<M, T> {
...
export function PropertyMetadataDecoratorFn<F, T>(
  manager: MetadataManager<PropertiesMetadata<F>, T>
): PropertyMetadataDecorator<F> {
